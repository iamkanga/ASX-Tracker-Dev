...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Create containers for summary and list
    const summaryBar = document.createElement('div');
    summaryBar.id = 'portfolioSummaryBar';
    summaryBar.className = 'portfolio-summary-bar';

    const listContainer = document.createElement('div');
    listContainer.id = 'portfolioListContainer';
    listContainer.className = 'portfolio-holdings-list';

    portfolioSection.appendChild(summaryBar);
    portfolioSection.appendChild(listContainer);

    const portfolioShares = allSharesData.filter(s => s.watchlistId === 'portfolio' && s.portfolioShares && s.portfolioAvgPrice);

    if (portfolioShares.length === 0) {
        listContainer.innerHTML = '<p>No shares in your portfolio yet.</p>';
        summaryBar.innerHTML = '<p>Your portfolio is empty.</p>';
        return;
    }
...
function addShareToMobileCards(share) {
    ...
    card.innerHTML = `
        ...
        <p class="data-row"><span class="label-text">Reference Price Entered Price ($):</span><span class="data-value">${(val => (val !== null && !isNaN(val) && val !== 0) ? '$' + val.toFixed(2) : '')(Number(share.currentPrice))}</span></p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    portfolioShares.forEach(share => {
        const shares = Number(share.portfolioShares);
        const priceNow = getDisplayPrice(share);
        const prevClose = livePrices[share.shareName.toUpperCase()]?.prevClose;

        if (!isNaN(shares) && priceNow !== null) {
            const holdingValue = shares * priceNow;
            totalPortfolioValue += holdingValue;

            if (prevClose !== null && !isNaN(prevClose)) {
                const holdingDayChange = (priceNow - prevClose) * shares;
                totalDayChange += holdingDayChange;
                totalInitialValue += shares * prevClose;
            } else {
                // If no previous close, assume no change for the day for this stock
                totalInitialValue += holdingValue;
            }
        }
    });

    const totalPercentageChange = totalInitialValue > 0 ? (totalDayChange / totalInitialValue) * 100 : 0;
    const summaryPLClass = totalDayChange > 0 ? 'positive' : (totalDayChange < 0 ? 'negative' : 'neutral');

    summaryBar.innerHTML = `
        <div class="summary-value">${fmtMoney(totalPortfolioValue)}</div>
        <div class="summary-change ${summaryPLClass}">
            <span>${fmtMoney(totalDayChange)}</span>
            <span>(${totalPercentageChange.toFixed(2)}%)</span>
        </div>
    `;
    ...
    portfolioShares.forEach(share => {
        ...
        if (!isNaN(shares) && priceNow !== null) {
            ...
            let holdingDayChangePct = 0;
            if (prevClose !== null && !isNaN(prevClose)) {
                holdingDayChange = (priceNow - prevClose) * shares;
                holdingDayChangePct = prevClose > 0 ? ((priceNow - prevClose) / prevClose) * 100 : 0;
            }
            
            const holdingPLClass = holdingDayChange > 0 ? 'positive' : (holdingDayChange < 0 ? 'negative' : 'neutral');

            holdingsHtml += `
                <div class="holding-item" data-doc-id="${share.id}">
                    <div class="holding-name">${share.shareName}</div>
                    <div class="holding-value">${fmtMoney(holdingValue)}</div>
                    <div class="holding-change ${holdingPLClass}">
                        <span>${fmtMoney(holdingDayChange)}</span>
                        <span>(${holdingDayChangePct.toFixed(2)}%)</span>
                    </div>
                </div>
            `;
        }
    ...
    listContainer.innerHTML = holdingsHtml;

    // Add event listeners
    listContainer.querySelectorAll('.holding-item').forEach(item => {
